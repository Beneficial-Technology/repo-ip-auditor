import { STRONG_COPYLEFT } from './licenses.js';
...
export function score(input) {
    ...
    if (strongPts)
        d.push({ label: plural(strong + gpl, 'strong copyleft runtime dependency', 'strong copyleft runtime dependencies'), points: strongPts });
    ...
    if (!input.hasRootLicense)
        d.push({ label: 'No root LICENSE file', points: 10 });
    else if (['agpl', 'gpl', 'lgpl'].includes(input.rootLicenseClass))
        d.push({ label: 'Repository root license is copyleft', points: 20 });
    const total = Math.max(0, 100 - d.reduce((s, x) => s + x.points, 0));
    return { score: total, grade: grade(total), deductions: d };
}
export const grade = (n) => n >= 90 ? 'A' : n >= 75 ? 'B' : n >= 60 ? 'C' : n >= 40 ? 'D' : 'F';
export function verdict(r) {
    if (r.score >= 90)
        return 'Chain of title looks clean on the evidence in this history. Keep assignment records current as the team grows.';
    if (r.score >= 75)
        return 'Mostly clean. A small number of items need paper before a diligence review, and each is cheap to fix now.';
    if (r.score >= 60)
        return 'Diligence stalls here. The gaps are ordinary and fixable, but chasing signatures after a term sheet takes weeks.';
