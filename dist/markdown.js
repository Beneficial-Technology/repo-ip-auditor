...
import { RUBRIC, verdict } from './score.js';
import { STATUS_LABEL } from './identity.js';
import { HEADER_KIND_LABEL } from './headers.js';
...
export function markdown(r) {
    ...
    if (r.deductions.length) {
        ...
        for (const d of r.deductions)
            out.push(`| ${d.label} | -${d.points} |`);
        out.push('');
    }
    if (risky.length) {
        out.push('### Contributors without assignment evidence', '', '| Identity | Commits | Active | Status |', '|---|---|---|---|');
        for (const x of risky.slice(0, 50))
            out.push(`| \`${x.email}\` | ${x.commits} | ${x.first.slice(0, 7)} to ${x.last.slice(0, 7)} | ${STATUS_LABEL[x.status]} |`);
        out.push('');
    }
    if (r.dependencies.flagged.length) {
        out.push('### Copyleft exposure', '', '| Package | Class | Declared | Scope |', '|---|---|---|---|');
        for (const d of r.dependencies.flagged.slice(0, 50))
            out.push(`| \`${d.name}\` | ${LICENSE_LABEL[d.cls]} | ${d.license ?? '—'} | ${d.scope}${d.direct ? '' : ', transitive'} |`);
        out.push('');
    }
    if (r.headers.findings.length) {
        ...
        if (r.headers.findings.length > 50)
            out.push(`| … | ${r.headers.findings.length - 50} more | |`);
        out.push('');
    }
