...
export function parseManifestText(base, text, src) {
    const out = [];
    const push = (name, ecosystem, dev = false) => {
        name = (name || '').trim();
    ...
    try {
        if (base === 'package.json') {
            ...
            for (const line of text.split('\n')) {
                ...
                const m = line.match(/^\s*([A-Za-z0-9_-]+)\s*=/);
                if (m)
                    push(m[1], 'cargo', /dev-dependencies$/.test(section));
            }
        }
        else if (base === 'composer.json') {
            const j = JSON.parse(text);
            Object.keys(j.require ?? {}).forEach(n => { if (n !== 'php' && !n.startsWith('ext-'))
                push(n, 'packagist'); });
