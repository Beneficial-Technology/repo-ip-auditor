...
function rootLicense(root) {
    ...
    try {
        ...
        return { name: basename(file), text: null };
    }
}
export async function audit(opts) {
    ...
    for (const m of manifests) {
        try {
            deps.push(...parseManifest(m, readFileSync(m, 'utf8'), root));
        }
        catch { /* skip */ }
    }
    ...
    if (opts.offline)
        warnings.push('Offline mode. Only licenses found on disk were resolved.');
    const unresolved = deps.filter(d => d.cls === 'unknown').length;
    if (unresolved > 10)
        warnings.push(`${unresolved} dependencies have no machine-readable license and were not scored.`);
    if (!manifests.length)
        warnings.push('No package manifests found. Dependency scoring did not run.');
    if (!deep)
        warnings.push('Header scanning was skipped. File-level findings are not included in the score.');
    if (headerScan.truncated)
        warnings.push('More than 20,000 source files are tracked. Header scanning stopped at the limit.');
    ...
    try {
        repo = git(['config', '--get', 'remote.origin.url'], root).replace(/\.git$/, '').split(/[:/]/).slice(-2).join('/');
    }
    catch { /* local only */ }
    return {
