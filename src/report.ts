...
const dim = (s: string) => c('2', s), bold = (s: string) => c('1', s);
const red = (s: string) => c('31', s), yellow = (s: string) => c('33', s), green = (s: string) => c('32', s);

const gradeColor = (g: string) => g === 'A' || g === 'B' ? green : g === 'C' ? yellow : red;

import { STATUS_LABEL } from './identity.js';

export function terminal(r: AuditReport): string {
  const L: string[] = [];
  ...
  if (r.deductions.length) {
    ...
    for (const d of r.deductions) L.push(`  ${red('-' + String(d.points).padEnd(3))} ${d.label}`);
    L.push('');
  }
  ...
  if (risky.length) {
    ...
    if (risky.length > 25) L.push(dim(`  and ${risky.length - 25} more`));
    L.push('');
  }
  ...
  if (r.dependencies.flagged.length) {
    ...
    if (r.dependencies.flagged.length > 25) L.push(dim(`  and ${r.dependencies.flagged.length - 25} more`));
    L.push('');
  }

  if (r.headers.findings.length) {
    L.push(dim('  FILE HEADERS'));
    const byKind = new Map<string, typeof r.headers.findings>();
    ...
    for (const [, group] of [...byKind.entries()].slice(0, 12)) {
      ...
      for (const f of group.slice(0, 3)) L.push(dim(`      ${f.path}:${f.line}`));
      if (group.length > 3) L.push(dim(`      and ${group.length - 3} more files`));
    }
    L.push('');
  }

  for (const w of r.warnings) L.push(`  ${yellow('!')} ${dim(w)}`);
  L.push('');
  if (r.scanDepth === 'deep') L.push(dim(`  ${r.headers.filesScanned} source files header-scanned`));
