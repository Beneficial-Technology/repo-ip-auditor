...
export function findManifests(root: string, max = 24): string[] {
  const out: string[] = [];
  const walk = (dir: string, depth: number) => {
    ...
    for (const e of entries) {
      ...
      if (s.isDirectory()) { if (!SKIP.test(e) && !e.startsWith('.')) walk(p, depth + 1); }
      else if (MANIFESTS.includes(e)) out.push(p);
    }
  };
  walk(root, 0);
...
export function parseManifest(path: string, text: string, root: string): Dependency[] {
...
export function readInstalledLicenses(root: string): Map<string, { license: string; ecosystem: Dependency['ecosystem'] }> {
  const map = new Map<string, { license: string; ecosystem: Dependency['ecosystem'] }>();

  const npmDir = (dir: string, depth: number) => {
    ...
    for (const e of entries) {
      ...
      const p = join(dir, e);
      if (e.startsWith('@')) { npmDir(p, depth); continue; }
      const pkg = join(p, 'package.json');
      if (existsSync(pkg)) {
        try {
          const j = JSON.parse(readFileSync(pkg, 'utf8'));
          const lic = typeof j.license === 'string' ? j.license
            : j.license?.type ?? (Array.isArray(j.licenses) ? j.licenses.map((x: any) => x.type).join(' OR ') : null);
          if (j.name && lic) map.set('npm:' + j.name, { license: lic, ecosystem: 'npm' });
        } catch { /* skip */ }
      }
      ...
      for (const e of entries) {
        ...
        if (e.endsWith('.dist-info')) {
          try {
            ...
            if (name && lic) map.set('pypi:' + name.toLowerCase(), { license: lic.trim(), ecosystem: 'pypi' });
          } catch { /* skip */ }
        } else if (!e.includes('.')) {
          try { if (statSync(p).isDirectory()) stack.push(p); } catch { /* skip */ }
        }
      }
    }
  }
...
async function remoteLicense(d: Dependency): Promise<string | null> {
  ...
  if (d.ecosystem === 'pypi') {
    ...
    if (!j) return null;
    if (j.info?.license_expression) return j.info.license_expression;
...
export async function resolveLicenses(deps: Dependency[], root: string, offline: boolean): Promise<Dependency[]> {
  ...
  for (const [key, v] of installed) {
    ...
    if (declared.has(v.ecosystem + ':' + name.toLowerCase())) continue;
  ...
  for (const d of deps) {
    ...
    if (hit) { d.license = hit.license; d.cls = classifyLicense(hit.license); }
  }
  ...
  for (const d of deps) {
    ...
    if (!prev || (prev.scope === 'dev' && d.scope === 'runtime') || (!prev.license && d.license)) seen.set(k, d);
  }
  return [...seen.values()];
}
