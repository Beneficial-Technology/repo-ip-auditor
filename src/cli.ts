...
function positional(argv: string[]): string {
  for (let i = 0; i < argv.length; i++) {
    if (argv[i].startsWith('-')) { if (VALUE_FLAGS.includes(argv[i])) i++; continue; }
    return argv[i];
  }
  return '.';
}
...
async function main() {
  ...
  if (argv.includes('--rubric')) {
    for (const r of RUBRIC) console.log(`-${String(r.per).padEnd(3)} ${r.label.padEnd(52)} cap ${r.cap}`);
  ...
  if (out) { writeFileSync(out, body); console.error(`Report written to ${out}`); }
