...
const input = (name: string) => process.env[`INPUT_${name.toUpperCase().replace(/-/g, '_')}`]?.trim() ?? '';
...
async function main() {
  ...
  if (summary) appendFileSync(summary, markdown(report) + '\n');
  ...
  if (!report.historyComplete) console.log('::warning::Shallow clone detected. Set fetch-depth: 0 on actions/checkout or the contributor list is incomplete.');
